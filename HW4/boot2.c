...
//new prototypes
void kbdEnqueue(pcb_t* thisPCB);
pcb_t* kbdDequeue();
void kbd_block();
void k_gets();
void kbdprint(char *string2print);
...
int main()
{
    ...
    //num_processes++;
    retVal = create_process((unsigned long) k_gets);

    //k_print("here", 4, 20, 10);
    //num_processes++;
...
void kbdprint(char *string2print)
{
    ...
    while(string_len != 0)
    {
        if(string_len < 80)
        {
            k_print(string2print, string_len, row2, col2);
            col2++;
            string_len = 0;
        }
        else
...
void kbdEnqueue(pcb_t* thisPCB) // find out what this should take
{
    int temp = kbd_q.qTail;
    kbd_q.qTail = (kbd_q.qTail+1) % 10;
    kbd_q.qPCBs[temp] = *thisPCB;
    kbd_qCOUNTER++;
}

pcb_t* kbdDequeue()
{
    int temp = kbd_q.qHead;
    kbd_q.qHead = (kbd_q.qHead+1) % 10;
    kbd_qCOUNTER--;
    return &(kbd_q.qPCBs[temp]);
}
...
void kbd_handler(unsigned int scancode)
{
    ...
    else
    {
        ...
        tail = tail % 256;
        bufferMax++;
        if(kbd_qCOUNTER != 0) // kbd_q is not empty
        {
            //dequeue the process from kbd_q and enqueue it to the ready queue
            pcb_t* temp = kbdDequeue();
            enqueue(temp);
        }
    }
