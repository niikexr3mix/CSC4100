...
int numPCBs = 0;
int numStacks = 0;
int* stackArray[maxProcesses][1024];
int stackSize = 1024 * maxProcesses;
int numProcesses = 0;
...
int main()
{
    ...
    int retVal = create_process(process_idle);
    numProcesses++;
...
int create_process(unsigned int processEntry)
{
    ...
    if(numPCBs >= 6 || numStacks >= 6)
    {
        return 1;
    }

    //set ds to the processes data selector (which is 8, same as OS for now)
    int ds = 8;

    //set stackptr to return value from allocate_stack()
    //  You will have the write allocat_state() yourself - NO MALLOC()
    int* stackptr = allocate_stack();
    ...
    int* st = stackptr + stackSize;
    st--; // subtract 1 from st

    //set a 32-bit word pointed at by st to the address of go() (can you guess why?)
    *st = (int) go;
    st--;//subtract 1 from st
    ...
    *st = 0x0200;
    ...
    //set 32-bit word pointed at by st to cs
    *st = cs;
    st--;
...
void enqueue(pcb_t* qThis)
{
    myQueue.qTail = (myQueue.tail+1) % queueSize;
    myQueue.qPCBs[myQueue.tail] = qTail
}
...
void p1()
{
    ...
    while(1)
    {
        msg = ("process p1: " + convert_num(i, msg));
